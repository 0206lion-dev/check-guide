...
// 때만 범위 밖으로 처리한다.
//
// 알려진 한계(수정 보류): "고수익"·"수익보장" 같은 미끼성 신호만으로도 위 우회가
// 발동해, "김해에 부동산 투자하면 무조건 200% 수익이래"처럼 순수 부동산 사기도
// 투자사기로 오분류될 수 있다. "리딩방"·"SNS"·"유사수신" 같은 금융 채널 신호가
// 함께 있을 때만 우회하도록 좁히는 방안을 검토했으나, "해외 부동산 개발에 투자하면
// 월 3% 배당 준다는데 사업자등록증은 보여주더라고요"(v4 라벨: 투자사기)처럼 채널
// 신호 없이 "배당"·"사업자등록증"만으로 투자사기가 성립하는 사례가 있어, 신호
// 구성만 보면 위 두 문장을 signals.json 23종 범위 안에서 구분할 방법이 없다(직접
// 확인함 — 두 문장 모두 데이터 기반 신호는 "고수익" 계열뿐이고 채널 신호가 전혀
// 없다). 새 키워드·신호를 추가하지 않는 범위에서는 이 오분류를 안전하게 막을 수
// 없다고 판단해 조건을 그대로 두었다.
...
// 조치 J — 일반인이 낯설어할 금융 용어에는 괄호로 쉬운 설명을 바로 붙인다(새
// 버튼·팝업·툴팁 없이). "확인대상" 문자열은 REASON_BY_TARGET/JUDGMENT_BY_TARGET의
// 조회 키이기도 해서, type_profiles.json에서 이 문자열에 괄호 설명을 추가한
// 두 항목(제도권 금융회사, 유사투자자문업자)은 아래 키도 정확히 같은 문자열로
// 맞췄다 — 하나만 고치면 조회가 어긋나 근거·판단 문구가 기본값으로 빠진다.
const REASON_BY_TARGET = {
  "제도권 금융회사(정부의 정식 인가를 받은 금융회사) 여부":
    "미인가 업체는 예금자보호법(금융회사가 망해도 일정 금액까지 돈을 돌려받을 수 있게 하는 법)과 금융분쟁조정 대상에서 제외됩니다. 문제가 생겨도 법적 구제를 받기 어렵습니다.",
  "유사투자자문업자(투자 정보만 제공할 수 있고 개별 종목 매수·매도를 대신 권유할 수 없는 업체) 등록 여부":
    "등록돼 있어도 특정 종목을 콕 집어 매매를 권유하거나 투자금을 대신 운용(투자일임)하는 것은 금지된 행위입니다. 등록 여부와 별개로 그 행위 자체가 위반입니다.",
  "가상자산사업자 신고 여부":
    "미신고 사업자를 이용하면 가상자산이용자보호법(가상자산 이용자의 예치금을 보호하는 법)의 예치금 분리보관·해킹 배상 등 보호를 받지 못합니다.",
  "대부업체 등록 여부":
    "미등록 대부업자에게 받은 대출은 이자제한법(법으로 정한 최고 이자율을 넘는 이자는 못 받게 하는 법)상 최고금리 초과분에 대해 무효를 주장할 수 있고, 미등록 영업 자체가 형사처벌 대상입니다.",
};
